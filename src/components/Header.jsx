  ...
  return (
    <>
      <header className="sticky top-3 z-50">
        {/* neon side bars */}
        <div aria-hidden className="pointer-events-none absolute inset-y-0 left-0 w-3 -z-10">
          <div className="neon-bar-left h-full" />
        </div>
        <div aria-hidden className="pointer-events-none absolute inset-y-0 right-0 w-3 -z-10">
        ...
        >
          <div className="flex items-center justify-between px-4 sm:px-6 lg:px-8 h-16">
            ...
            <Link to="/" className="flex items-center gap-3" onClick={closeMenu}>
              <div
                ...
                style={{
                  background: "linear-gradient(135deg, rgba(14,165,233,0.09), rgba(124,58,237,0.08))",
          ...
          <div
            ...
            className={`md:hidden overflow-hidden transition-[max-height,opacity,transform] duration-400 ease-in-out ${
              isOpen ? "max-h-[420px] opacity-100 translate-y-0" : "max-h-0 opacity-0 -translate-y-2"
            }`}
            aria-hidden={!isOpen}
          >
            <div className="px-4 pb-6 pt-3" role="menu" aria-label="Mobile Navigation">
              <div className="flex flex-col gap-3">
                <Link to="/" onClick={closeMenu} className="py-2 px-3 rounded-md text-white/95 hover:bg-white/6 transition">Home</Link>
                <Link to="/games" onClick={closeMenu} className="py-2 px-3 rounded-md text-white/95 hover:bg-white/6 transition">Games</Link>
                <Link to="/schedule" onClick={closeMenu} className="py-2 px-3 rounded-md text-white/95 hover:bg-white/6 transition">Schedule</Link>
                <Link to="/participants" onClick={closeMenu} className="py-2 px-3 rounded-md text-white/95 hover:bg-white/6 transition">Participants</Link>
                <Link to="/about" onClick={closeMenu} className="py-2 px-3 rounded-md text-white/95 hover:bg-white/6 transition">About</Link>
                ...
                <div className="pt-2">
                  <Link
                    ...
                    style={{ background: "linear-gradient(90deg,#06b6d4,#7c3aed)", color: "#071124" }}
                  >
                    Register Now
                  </Link>
                </div>
...
function NavLink({ to, children }) {
  return (
    <Link to={to} className="text-white/90 hover:text-white font-medium transition">
      {children}
    </Link>
  );
}
