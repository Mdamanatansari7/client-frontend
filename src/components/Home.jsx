...
//   // typed title
//   const typed = useTyping("LOGICAL 2025", 70, true);
...
// HomeAnimated.optimized.jsx
import React, { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { Link } from "react-router-dom";
import { motion, AnimatePresence } from "framer-motion";
import Footer from "./footer";

// Use the uploaded file path as the trophy/logo asset (developer-provided local path)
const TROPHY_SRC = "../../public/image/trofi.png";
...
/* -------------------------
   Lightweight typing hook
   - uses setInterval but clears properly
   ------------------------- */
function useTyping(text, speed = 70, loop = false) {
  const [display, setDisplay] = useState("");
  const iRef = useRef(0);
  useEffect(() => {
    let mounted = true;
    ...
    const tick = () => {
      if (!mounted) return;
      iRef.current++;
      setDisplay((prev) => text.slice(0, iRef.current));
      if (iRef.current >= text.length) {
        if (!loop) return;
    ...
    // run immediately so first char shows without waiting
    tick();
    return () => {
      mounted = false;
      clearInterval(id);
...
/* -------------------------
   Countdown logic
   - single interval, minimal state updates
   ------------------------- */
function useCountdown(targetIso) {
  const targetRef = useRef(new Date(targetIso).getTime());
  const [timeLeft, setTimeLeft] = useState(() => computeTimeLeft(targetRef.current));

  useEffect(() => {
    const tick = () => {
      const next = computeTimeLeft(targetRef.current);
      // avoid updating if values didn't change (reduce re-renders)
      setTimeLeft((prev) => {
        if (
          prev.days === next.days &&
    ...
    // align to exact second boundary for smoother ticks
    const now = Date.now();
    const msToNextSecond = 1000 - (now % 1000);
    const startTimeout = setTimeout(() => {
      tick();
      const id = setInterval(tick, 1000);
      // keep interval id in ref for cleanup via closure
      (startTimeout as any).intervalId = id;
    }, msToNextSecond);
    return () => {
      clearTimeout(startTimeout);
      clearInterval((startTimeout as any).intervalId);
    };
  }, [targetIso]);
...
/* -------------------------
   Smaller subcomponents (memoized)
   ------------------------- */
const Tile = React.memo(function Tile({ value, label, tick }) {
  return (
    <motion.div
      layout
      whileHover={{ scale: 1.02 }}
      className="relative rounded-xl p-4 bg-gradient-to-br from-white/6 to-white/3 border border-white/8 tile-pop"
      style={{ boxShadow: "inset 0 -6px 18px rgba(255,255,255,0.02), 0 8px 24px rgba(12,74,120,0.06)" }}
    >
      <motion.div animate={{ scale: tick ? 1.03 : 0.995 }} transition={{ duration: 0.18 }} className="text-2xl sm:text-3xl font-extrabold text-white/95" aria-live="polite">
        {format(value)}
      </motion.div>
      <div className="mt-1 text-xs sm:text-sm text-slate-200/70 uppercase tracking-wider">{label}</div>
      <div className="absolute -top-3 -right-3 w-9 h-9 rounded-full opacity-10" style={{ background: "conic-gradient(from 90deg, rgba(255,255,255,0.12), transparent 40%)", filter: "blur(6px)" }} />
    </motion.div>
  );
});

const AchievementCard = React.memo(function AchievementCard({ item }) {
  return (
    <div className="bg-white/4 p-4 rounded-xl border border-white/8" style={{ boxShadow: "0 6px 20px rgba(14,165,233,0.04)" }}>
      <div className="flex items-center justify-between">
        <div>
          <div className="text-2xl font-bold text-sky-200">{item.year}</div>
          <div className="text-sm text-slate-200/80">{item.title}</div>
          <div className="text-xs text-slate-300/70 mt-1">{item.note}</div>
        </div>
        <div className="text-right">
          <div className="text-sm uppercase text-slate-300">{item.rank}</div>
          <div className="mt-2 inline-flex items-center gap-2">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${item.medal === "gold" ? "bg-yellow-400" : item.medal === "silver" ? "bg-slate-300" : "bg-amber-700"}`}>
              <svg className="w-4 h-4 text-white" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l2.6 5.2L20 8l-4 3.5L17 18l-5-2.6L7 18l1-6.5L4 8l5.4-.8L12 2z"/></svg>
            </div>
          </div>
        </div>
...
export default function HomeAnimated() {
  const typed = useTyping("LOGICAL 2025", 70, true);
  const timeLeft = useCountdown(TARGET_DATE_ISO);
  const [tick, setTick] = useState(false); // used for subtle tile animation
  const [index, setIndex] = useState(0);
  const isLive = Date.now() >= new Date(TARGET_DATE_ISO).getTime();

  // Auto-advance achievements with a single interval and low frequency
  useEffect(() => {
    const id = setInterval(() => setIndex((s) => (s + 1) % ACHIEVEMENTS.length), 3500);
    return () => clearInterval(id);
  }, []);

  // minor tick toggler synced to seconds (keeps tile animation in sync)
  useEffect(() => {
  ...
  // precompute current achievement item
  const currentAchievement = useMemo(() => ACHIEVEMENTS[index], [index]);

  // prefer-reduced-motion detection for accessibility
  const [reducedMotion, setReducedMotion] = useState(false);
  useEffect(() => {
    const q = window.matchMedia && window.matchMedia("(prefers-reduced-motion: reduce)");
    const handler = () => setReducedMotion(q && q.matches);
    handler();
    q && q.addEventListener && q.addEventListener("change", handler);
    return () => q && q.removeEventListener && q.removeEventListener("change", handler);
  }, []);
  ...
  return (
    <>
      <section className="relative w-full min-h-screen flex items-center justify-center py-20 px-4 bg-gradient-to-b from-[#071033] via-[#072b4f] to-[#071028] text-white overflow-hidden">
        ...
        {/* Background layers (cheap, big blurs, simple animations) */}
        <div aria-hidden className="absolute inset-0 -z-10">
          <div className={`absolute -left-56 -top-24 w-[52rem] h-[52rem] rounded-full opacity-30 ${reducedMotion ? "" : "animate-blob-slow"}`} style={{ background: "radial-gradient(circle at 20% 25%, rgba(14,165,233,0.18), transparent 18%), radial-gradient(circle at 70% 75%, rgba(124,58,237,0.12), transparent 25%)", filter: "blur(110px)", transform: "rotate(-12deg)" }} />
          <div className={`absolute -right-56 -bottom-24 w-[52rem] h-[52rem] rounded-full opacity-24 ${reducedMotion ? "" : "animate-blob-slow delay-2500"}`} style={{ background: "radial-gradient(circle at 30% 30%, rgba(59,130,246,0.16), transparent 18%), radial-gradient(circle at 70% 70%, rgba(6,182,212,0.12), transparent 25%)", filter: "blur(120px)", transform: "rotate(10deg)" }} />
          <div className="absolute left-1/2 top-1/3 -translate-x-1/2 -translate-y-1/2 w-[60rem] h-[40rem] rounded-full pointer-events-none opacity-36" style={{ background: "radial-gradient(circle at 50% 35%, rgba(96,165,250,0.14), rgba(14,165,233,0.08) 20%, transparent 45%)", filter: "blur(90px)" }} />
          <div className="absolute inset-0 bg-gradient-to-b from-transparent to-black/60" />
        </div>
        ...
        <div className="max-w-6xl w-full mx-auto z-10 px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            ...
            <div className="space-y-6 text-center lg:text-left">
              ...
              <h1 className="text-4xl sm:text-5xl md:text-6xl font-extrabold leading-tight tracking-tight">
                <span className="block text-transparent bg-clip-text bg-gradient-to-r from-sky-300 via-cyan-200 to-indigo-400">
                  {typed}
                  <span className="ml-1 inline-block animate-pulse" aria-hidden>│</span>
                </span>
              </h1>
            ...
            {/* Right: countdown + achievements */}
            <div>
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="bg-white/6 backdrop-blur-md border border-white/8 rounded-2xl p-6 shadow-2xl" style={{ boxShadow: '0 18px 40px rgba(2,6,23,0.5), 0 6px 30px rgba(14,165,233,0.04)' }}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                  {tiles.map((t, i) => <Tile key={t.label} value={t.value} label={t.label} tick={tick} />)}
                </div>
                ...
                <div className="mt-2 grid grid-cols-1 sm:grid-cols-3 gap-3 items-center">
                  <div className="col-span-1 flex items-center justify-center relative">
                    <div className="absolute -inset-6 rounded-full blur-3xl opacity-60" style={{ background: 'radial-gradient(circle at 50% 45%, rgba(56,189,248,0.18), rgba(99,102,241,0.06) 30%, transparent 55%)' }} />
                    <div className="w-32 h-32 perspective-1000 relative z-10">
                      {/* gentle repeating float animation but reduced intensity */}
                      <motion.div animate={reducedMotion ? {} : { rotateY: [0, 6, -6, 0], y: [0, -4, -2, 0] }} transition={{ duration: 3.8, repeat: Infinity, ease: "easeInOut" }} className="relative w-full h-full transform-style-preserve-3d">
                        <img src={TROPHY_SRC} alt="trophy" className="w-full h-full object-contain drop-shadow-3xl" width={128} height={128} loading="lazy" decoding="async" />
                        <div className="absolute inset-0 rounded-full pointer-events-none" style={{ background: 'linear-gradient(120deg, rgba(255,255,255,0.12), transparent)' }} />
                      </motion.div>
                    </div>
                  </div>
                  ...
                  <div className="col-span-2">
                    <div className="overflow-hidden">
                      <AnimatePresence initial={false} mode="wait">
                        <motion.div key={index} initial={{ opacity: 0, x: 18 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -18 }} transition={{ duration: 0.5 }} >
                          <AchievementCard item={currentAchievement} />
                        </motion.div>
                      </AnimatePresence>
                    </div>
                  </div>
        ...
        <style>{`
          @keyframes blob { 0% { transform: translateY(0) scale(1) rotate(-12deg); } 33% { transform: translateY(-20px) scale(1.03) rotate(-8deg); } 66% { transform: translateY(10px) scale(0.98) rotate(-14deg); } 100% { transform: translateY(0) scale(1) rotate(-12deg); } }
          .animate-blob-slow { animation: blob 18s ease-in-out infinite; }
          ...
          @media (prefers-reduced-motion: reduce) {
            .animate-blob-slow, .drop-shadow-3xl { animation: none !important; filter: none !important; }
          }
        `}</style>
      </section>
